...
### 2-1. GROWTH_PROFILE.md 읽기

`D:\kyra-test\projects\vibelog\GROWTH_PROFILE.md` 파일을 읽습니다.

이 파일 하나에 성장 맥락이 압축되어 있어서, entries가 아무리 쌓여도 토큰을 일정하게 유지할 수 있습니다.

**파일이 없는 경우:** 입문 수준으로 시작하고, Phase 4 저장 후 파일을 새로 생성합니다.

### 2-2. 성장 수준 판단

GROWTH_PROFILE.md의 `현재 수준` 필드를 사용합니다.

| 수준 | 기준 | 전문가 요약 톤 |
|------|------|----------------|
| 입문 (1~5개) | 환경 설정, 첫 배포 경험 | "처음 접하는 기술로 ~를 구현" |
| 초급 (6~15개) | CRUD, 기본 연동, UI | "~를 활용해 ~를 구현, 핵심은 ~" |
| 중급 (16~30개) | 설계 결정, 최적화, 복잡한 기능 | "~를 적용해 ~를 해결. 트레이드오프: ~" |
| 고급 (31개+) | 아키텍처, 성능, 팀 작업 | "~를 채택한 이유: ~. 대안 대비 장점: ~" |

### 2-3. GROWTH_PROFILE.md 업데이트 (Phase 4에서 수행)
...
- **총 entries**: N개
- **현재 수준**: 입문 / 초급 / 중급 / 고급
- **반복 사용 기술**: Next.js, Supabase, TypeScript, ...
- **최근 주제**: (오늘 작업 핵심 키워드)
- **연속 진행 프로젝트**: (같은 프로젝트를 이어가고 있으면 기록)
- **마지막 업데이트**: YYYY-MM-DD
```

업데이트 규칙:
- `총 entries`: +1
- `현재 수준`: entries 수 기준으로 자동 조정
- `반복 사용 기술`: 오늘 사용한 기술 중 기존 목록에 없는 것 추가
- `최근 주제`: 오늘 핵심 작업으로 교체
- `연속 진행 프로젝트`: 같은 프로젝트면 유지, 새 프로젝트면 교체
